...
export const DocumentFilters = ({
  ...
  return <Card className="border-0 shadow-sm bg-gradient-to-r from-muted/30 to-muted/10">
      <CardContent className="py-2 md:py-4 px-2 md:px-6 bg-[#f4f0f0]">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="grid grid-cols-4 md:flex md:flex-wrap gap-1.5 md:gap-3 flex-1 items-center">
            ...
            <div className="flex flex-col">
              ...
              <Input type="date" value={filters.dateFrom} onChange={e => handleFilterChange("dateFrom", e.target.value)} className="h-7 md:h-8 text-[10px] md:text-sm px-1.5 md:px-3 w-full md:w-32" />
            </div>

            {/* Date To */}
            <div className="flex flex-col">
              <Label className="text-[9px] md:text-xs text-muted-foreground mb-0.5">To</Label>
              <Input type="date" value={filters.dateTo} onChange={e => handleFilterChange("dateTo", e.target.value)} className="h-7 md:h-8 text-[10px] md:text-sm px-1.5 md:px-3 w-full md:w-32" />
            </div>

            {/* Client Filter */}
            {showClientFilter && <div className="flex flex-col">
                <Label className="text-[9px] md:text-xs text-muted-foreground mb-0.5">Client</Label>
                <Select value={filters.clientId} onValueChange={value => handleFilterChange("clientId", value === "all" ? "" : value)}>
